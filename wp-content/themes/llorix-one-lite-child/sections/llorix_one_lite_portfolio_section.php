 ...
 */
	...
	$portfolio=array();
	foreach ($pf as $item) {
		$cat = get_field("category", $item->ID);
		
		$category=array();
		foreach ($cat as $c){
			$category = $c->name; // Array delle categorie
		}
	...
	echo '<div class="progetti dark-text"><h3>Progetti</h3></div>';
	echo '<div class="colored-line-center"></div>';
